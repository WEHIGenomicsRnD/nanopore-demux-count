process TrimPrimer {
    ...
    publishDir "${params.outdir}/${primerName}", mode: 'copy'
...
//    tuple val(sampleName), path(fastq)
    tuple val(primerName), val(fwd_primer), val(rev_primer), val(sampleName), path(fastq)
    val mismatches
    val barcode_length
    val output_untrimmed

    output:
    tuple val(primerName) ,val(fwd_primer), val(rev_primer) , val(sampleName), path("*_trimmed.fastq*"), emit: trimmed_ch
